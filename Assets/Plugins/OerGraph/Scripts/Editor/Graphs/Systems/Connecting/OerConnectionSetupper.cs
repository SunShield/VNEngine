...
namespace OerGraph.Editor.Graphs.Systems.Connecting
{
    public static class OerConnectionSetupper
    {
        public static void SetupNewConnection(Edge edge, OerGraphView graphView)
        {
            ...
            if (CheckRuntimeConnectionExists(graphView.GraphAsset, port1Id, port2Id)) return;
            AddRuntimeConnection(graphView.GraphAsset, port1Id, port2Id);

            SetupConnectionInternal(port1, port2, edge, graphView);
        }
        
        private static bool CheckRuntimeConnectionExists(OerGraphAsset graph, int port1Id, int port2Id) => graph.Graph.CheckConnectionExists(port1Id, port2Id);
        private static void AddRuntimeConnection(OerGraphAsset graph, int port1Id, int port2Id) => graph.Graph.AddConnection(port1Id, port2Id);
