...
{
    public class EqualsNode : OerNode
    {
        protected override OerNode CreateInstance() => new EqualsNode();
        ...
        public override OerNodePortsData GetPortsData()
        {
            return new(new()
            {
                ...
                ("Float", "Delta",   OerPortType.Input,  false),
        ...
        public override object GetValue(string portName)
        {
            ...
            var delta = GetPort<OerFloatPort>("Number2").GetValue();

            return (number1 - number2) <= delta;
        }
    }
}